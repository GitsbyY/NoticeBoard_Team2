...
VALUES(USER_INFO_USER_NO_SEQ.NEXTVAL, 'ID', 'PW', 'test@email.com', 'name', 'nick', SYSDATE, SYSDATE);
...
DROP SEQUENCE BOARD_POST_NUMBER_SEQ; --게시판 번호 시퀀스 삭제

CREATE TABLE BOARD (
    USER_EMAIL VARCHAR2(40) NOT NULL,
    POST_NUMBER NUMBER NOT NULL,
    POST_TITLE VARCHAR2(100) NOT NULL,
    POST_NICKNAME VARCHAR2(50) NOT NULL,
    POST_DATE DATE NOT NULL,
    POST_PWD VARCHAR2(20) NOT NULL,  
    POST_CONTENT VARCHAR2(900) NOT NULL,
    POST_VIEW_NUMBER NUMBER NOT NULL
);

COMMENT ON COLUMN BOARD.USER_EMAIL IS '이메일';
COMMENT ON COLUMN BOARD.POST_NUMBER IS '게시글번호';
COMMENT ON COLUMN BOARD.POST_TITLE IS '제목';
COMMENT ON COLUMN BOARD.POST_NICKNAME IS '글쓴이';
...
COMMENT ON COLUMN BOARD.POST_VIEW_NUMBER IS '조회수';


ALTER TABLE BOARD
ADD CONSTRAINT USER_INFO_USER_EMAIL_FK FOREIGN KEY(USER_EMAIL) REFERENCES USER_INFO(USER_EMAIL);

CREATE SEQUENCE BOARD_POST_NUMBER_SEQ
INCREMENT BY 1
START WITH 1;

INSERT INTO board
VALUES('name', 'pw', 'test@email.com', BOARD_POST_NUMBER_SEQ.nextval, 'title', 'content');
